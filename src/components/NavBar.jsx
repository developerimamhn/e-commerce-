...
const NavBar = ({ searchTerm, setSearchTerm, cart }) => {
  ...
  return (
    <nav
      className={` header ${
        scrolled ? "scrolled" : " "
      } z-[150] w-full header backgronsdvg   ${
        isVisible
      ...
      <div className="flex items-center justify-between container mx-auto py-3  sm:px-0 px-[24px] taoalnumbering">
        <div className="flex items-center gap-4">
          <div className="dropdown" tabIndex="0">
            <button id="dropdown-btn" className="flex items-center gap-2">
              <img
                src={getFlagSrc(intlLocale.region)}
                alt={langName}
                className="w-5 h-5"
              />
              {langName}
          ...
          <p className="text-gray-700 lg:block hidden text-[10px] sm:text-[11px] md:text-[12px] lg:text-[13px] xl:text-[14px] 2xl:text-[15px]">
            Mail: webzedcontact@gmail.com
          </p>
          <p className="text-gray-700 sm:block hidden text-[10px] sm:text-[11px] md:text-[12px] lg:text-[13px] xl:text-[14px] 2xl:text-[15px]">
            Helpline: 4534345656
          </p>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          >
            ...
            <span class="absolute w-36 h-20 -top-8 -left-2 bg-white rotate-12 transform scale-x-0 group-hover:scale-x-100 transition-transform group-hover:duration-500 duration-1000 origin-left"></span>
            <span class="absolute w-36 h-20 -top-8 -left-2 bg-purple-400 rotate-12 transform scale-x-0 group-hover:scale-x-100 transition-transform group-hover:duration-700 duration-700 origin-left"></span>
            <span class="absolute w-36 h-20 -top-8 -left-2 bg-purple-600 rotate-12 transform scale-x-0 group-hover:scale-x-100 transition-transform group-hover:duration-1000 duration-500 origin-left"></span>
            <span class="group-hover:opacity-100 text-center leading-[110%] group-hover:duration-1000 duration-100 opacity-0 absolute top-2.5 left-6 z-10">
              {isLoggedIn ? "Logout" : "Log in"}
            </span>
          </button>
          <div
